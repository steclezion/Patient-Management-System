...
if ((in_array('5', $user_permission))) {
	...
	$query_task_tracker = "SELECT * FROM task_tracker where id = '$Get_transaction_id '  ";
    // mysqli select query
...
<form method="post" id="create_invoice">
	...
	$search = ',';
	$replace = ' ';
	$subject = $Get_tasks['Test_Type'];
	$test = str_replace($search, $replace, $subject); 


	$Get_main_task = explode(',',$Get_tasks['main_task']);
	if($Get_main_task[0] ==0 ) { $Labratory_Test .= '';} else { $Labratory_Test .= '<b> GENERAL TEST- </b>[<i style="color:orange">'.$test.'&nbsp;</i> ]<br>';}
