...
<?PHP $dt = new DateTime(); 
	 ...
	 $count_patient= "SELECT count(id) as count FROM store_customers";
	 $results = $mysqli->query($count_patient);
	 $counted_numbers  =   $results->fetch_assoc();
	 $Format_id =   $counted_numbers['count']+1;
	 $zero_filled_counter = sprintf('%04d',$Format_id);
	 $counted_id = PID."-".$zero_filled_counter;
							   ?>
<input type="text" readonly value="<?php  echo $counted_id ?>" class="form-control margin-bottom copy-input required" name="patient_id"   id="patient_id" placeholder="<?php  echo $counted_id  ?>" tabindex="1">
</div>
